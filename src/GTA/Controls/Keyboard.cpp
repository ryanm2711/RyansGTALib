...
void GTA::Controls::OnKeyboardMessage(DWORD key, WORD repeats, BYTE scanCode, BOOL isExtended, BOOL isWithAlt, BOOL wasDownBefore, BOOL isUpNow)
{
	if (key < KEYS_SIZE)
	{
...
bool GTA::Controls::IsKeyDown(DWORD key)
{
	return (key < KEYS_SIZE) ? ((GetTickCount() < keyStates[key].time + MAX_DOWN) && !keyStates[key].isUpNow) : false;
}

bool GTA::Controls::IsKeyJustUp(DWORD key, bool exclusive)
{
...
void GTA::Controls::ResetKeyState(DWORD key)
{
	if (key < KEYS_SIZE)
